# RealtimeTalk — AI-Powered Language Learning Through Voice

<p align="center">
  <strong>A 100% client-side web application for real-time voice conversations with OpenAI AI models, featuring an integrated language tutor with quizzes, roleplays, spaced repetition, and gamification — connecting via WebRTC directly from the browser.</strong>
</p>
...
**RealtimeTalk** is a web application that lets you have real-time voice conversations with OpenAI's AI models. The default experience is **Sofia**, an AI English tutor that adapts to your level, tracks your progress, and offers interactive exercises — all through natural voice conversation. Your microphone audio is transmitted directly to OpenAI via WebRTC, and the model's voice response is played back in the browser — no backend, no remote database, no server-side authentication.
...
- **Voice per personality** — each personality can define its own voice; switching personalities automatically updates the AI voice
- **Configurable semantic VAD** — semantic end-of-speech detection with adjustable aggressiveness (low, medium, high, auto)
- **Smart speech capture** — exercise-aware VAD that automatically reduces sensitivity during quizzes and exercises to prevent false interruptions
- **Real-time transcription** — bidirectional transcription (user and model) displayed during conversation

### Language Tutor (Sofia)
- **Dedicated teacher interface** — purpose-built page with welcome screen, activity picker, and gamification dashboard
- **Adaptive level assessment** — automatic CEFR placement test (A1-C2) at first session, with difficulty adapting to your level
- **7 interactive activities**:
  - **Free Talk** — open-ended conversation practice
  - **Vocabulary Quiz** — word-by-word quizzes with spaced repetition tracking
  - **Multiple Choice Quiz** — grammar, idioms, and vocabulary questions with interactive answer buttons
  - **Pronunciation Drills** — repeat-after-me exercises with phrase display
  - **Dictation** — listen-and-type exercises with similarity scoring
  - **Roleplay Scenarios** — 8 real-world situations (restaurant, airport, hotel, job interview, doctor, shopping, phone call, meeting) with in-character AI and performance scorecards
  - **Debate** — argumentation practice on various topics
- **Interactive challenge panel** — auto-detects quizzes from conversation and shows clickable answer buttons, pronunciation targets, or dictation indicators
- **Quick action bar** — one-tap shortcuts to start any activity mid-session
- **Spaced repetition flashcards** — SM-2 algorithm for optimal review scheduling of new vocabulary
- **Grammar correction** — gentle inline corrections with logging for progress tracking
- **Immersion mode** — AI speaks only in the target language
- **Deferred correction mode** — accumulate corrections for later review during fluency practice
- **Student profile tracking** — CEFR level, vocabulary count, grammar error rate, exercise scores stored in IndexedDB
- **Daily expression** — new idiom/phrase each session with usage monitoring
- **Guided tutorial** — step-by-step onboarding overlay for first-time users

### Gamification
- **Points system** — earn points for completed sessions (+10), perfect quizzes (+25), first roleplay (+15), and more
- **Streak tracking** — consecutive day tracking with streak display
- **Level badge** — CEFR level prominently displayed in the header
...
- **3 personality presets** — Default Assistant (Marin voice), Tech Support (Alex, Marin voice), Language Tutor (Sofia, Coral voice)
- **Personality compiler** — converts `PersonalityConfig` to an optimized system prompt with guardrails
- **Voice per personality** — each personality defines a preferred voice that is applied automatically when selected
...
- **50+ built-in actions** — vocabulary quizzes, grammar logging, roleplays, flashcards, gamification, student profiling, dictation, pronunciation, and more
- **Complete function calling** — model calls tool → handler executes → result returns → model continues speaking
- **Background actions** — actions that execute without injecting results into the conversation (e.g., analytics, progress tracking)
...
- **Student profile injection** — CEFR level, scores, and focus areas injected at session start for adaptive difficulty
...
### Data & Security
- **Session history** — list of past sessions with transcription, duration, tutor reports, and metadata
...
- **7 IndexedDB stores** — sessions, memories, student_profile, vocabulary, corrections, flashcards, gamification
...
- **Audio visualizer** — canvas with real-time frequency analysis via Web Audio API
...
The main page of RealtimeTalk is the **Sofia English Tutor** — a dedicated language learning interface built on top of the real-time voice engine.

### How It Works

1. **First visit**: A guided tutorial introduces the interface
2. **Setup**: Enter your OpenAI API key in Settings (gear icon)
3. **Choose an activity**: Pick from the welcome screen (Free Talk, Quiz, Roleplay, etc.)
4. **Learn through conversation**: Sofia adapts to your CEFR level and provides gentle corrections
5. **Track progress**: Points, streaks, and level badges update automatically

### Exercise-Aware Speech Detection

During quizzes and exercises, the app automatically adjusts Voice Activity Detection (VAD) to prevent false interruptions:
- **Low VAD eagerness by default** — waits longer before interpreting sounds as speech
- **Auto-disable interruption during exercises** — when a quiz or pronunciation drill is detected, the AI cannot be accidentally interrupted by ambient noise
- **Configurable in settings** — adjust VAD sensitivity via the Settings drawer

### Spaced Repetition

New vocabulary is automatically saved as flashcards using the **SM-2 algorithm**:
- Correct answers increase review interval (1 → 6 → 15 → 38 days...)
- Incorrect answers reset to 1 day
- Ease factor adjusts based on performance (min 1.3)
- Review sessions available via "Flashcards" activity
...
2. A tutorial overlay will guide you through the interface
3. Click the **Settings** icon (gear) in the top-right corner
4. Enter your **OpenAI API key** (starts with `sk-`)
5. Click **Save** to store it in memory
6. (Optional) Enter a passphrase and click **Encrypt & Save** for persistent encrypted storage
7. Close settings and choose an activity (or click **Start Conversation** for free talk)
8. Allow microphone access when prompted
9. Start speaking — Sofia will respond in real time!
...
| **IndexedDB** | Structured persistence (sessions, memories, student data, flashcards, gamification) |
...
│  │  Sessions │ Memories │ Student Profile │ Vocabulary │ Flash-  │   │
│  │  cards │ Corrections │ Gamification │ API Keys │ Personalities│   │
│  └──────────────────────────────────────────────────────────────┘   │
└──────────────────────────┬──────────────────────────────────────────┘
                           │
          ┌────────────────┴────────────────┐
...
|---|---|---|
| `/` | `TeacherPage` | Sofia English Tutor — main learning interface |
| `/settings` | `SettingsPage` | API key management and settings |
| `/history` | `HistoryPage` | Session history, tutor reports, and export/import |
...
│   ├── molecules/                # Combinations of atoms + LevelBadge, StreakCounter, PointsDisplay
│   ├── organisms/                # Complex UI sections + TeacherHeader, ChallengePanel, QuickActionsBar, WelcomeScreen, TutorialOverlay, TeacherSettingsDrawer
│   ├── templates/                # Page layout wrappers
│   └── pages/                    # Page components (Controller + View)
│       ├── TeacherPage/          # Sofia language tutor (main page)
│       ├── ConversationPage/     # General-purpose conversation
│       ├── HistoryPage/
│       ├── SettingsPage/
│       └── FaqPage/
├── hooks/                         # Shared React hooks
├── core/                          # Engine and infrastructure
├── actions/                       # Action Registry + 50+ handlers + quiz data
│   └── data/                     # Vocabulary bank (600+ words) + grammar quiz data
├── personality/                   # Personality system + roleplay scenarios
├── storage/                       # Local persistence (7 IndexedDB stores)
├── utils/                         # Utilities (cost estimator, SRS, text similarity, exercise detector)
...
- **Manual selection**: A language selector is available in the teacher header and settings page
...
- **Portugues**: [ACTION_TRIGGERS_PT.md](./docs/ACTION_TRIGGERS_PT.md)
...
A 100% client-side web app for real-time voice conversations with OpenAI's AI models via WebRTC, featuring an integrated language tutor with quizzes, roleplays, and spaced repetition.
...
### How does the language tutor work?
Sofia (the AI tutor) uses OpenAI's Realtime API for voice conversation and 50+ function-calling actions to deliver quizzes, roleplays, grammar correction, and progress tracking — all through natural speech.

### Does it remember my progress?
Yes. Student level, vocabulary, quiz results, grammar corrections, flashcards, points, and streaks are all stored locally in IndexedDB and persist across sessions.
